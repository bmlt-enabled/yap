...
if (($digits == 1 || $digits == 3) && count($sms_messages) > 0) {
    if (setting("sms_combine")) {
        $message = $GLOBALS['twilioClient']->messages->create($_REQUEST['From'], array("from" => $_REQUEST['To'], "body" => implode("\n\n", $sms_messages)));
    } else {
        for ($i = 0; $i < count($sms_messages); $i++) {
            $message = $GLOBALS['twilioClient']->messages->create($_REQUEST['From'], array("from" => $_REQUEST['To'], "body" => $sms_messages[$i]));
        }
    }
}
